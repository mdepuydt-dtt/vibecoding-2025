...
## Features Overview
This e-commerce webshop application provides a complete online shopping experience with the following features:
- **🛍️ Product Catalog** - Browse products across multiple categories (Electronics, Sports, Home, Fashion)
- **🛒 Shopping Cart** - Add items to cart with quantity management and persistent storage
- **🏷️ Category Filtering** - Filter products by category with dynamic category buttons
- **💳 Checkout Process** - Complete order flow from cart to checkout and order confirmation

# Getting Started - Copilot
- Navigate to the folder of the app: `cd webshop`
...
- Change the app into something else (e.g instead of a Webshop, make it into a Auction house)
